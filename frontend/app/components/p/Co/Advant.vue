<template>
    <BlockLiquidTitle
        v-if="props.title && props.cards.length"
        :id="slugify(props.title)"
        :title="props.title"
        :subtitle="props.subtitle"
        class="co-advant"
    >
        <ul class="co-advant__body">
            <li v-for="(card, idx) in props.cards" :key="idx" class="co-advant__item">
                <span class="co-advant__item-icon">
                    <SvgSprite :type="icons[idx % icons.length] ?? ''" :size="64" />
                </span>
                <h3 class="co-advant__item-title">{{ card.title }}</h3>
                <p class="co-advant__item-desc">{{ card.content }}</p>
            </li>
        </ul>
    </BlockLiquidTitle>
</template>
